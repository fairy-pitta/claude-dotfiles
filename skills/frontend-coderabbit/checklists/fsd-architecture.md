...
- **Markdownテーブル構造の保全** `[新観点 from PR#670]` — テーブル行間に非テーブル要素（blockquote, 段落等）を挿入していないか確認する。Markdownテーブルは行が連続していないとレンダリングが崩れる。注記はテーブル直後に配置する。
- **権限説明文のマスターデータ照合** `[新観点 from PR#670]` — 権限の説明文がバックエンドCSV定義および実装上の用途と一致しているか確認する。特に権限の粒度が変更された場合（例: manage → ページアクセスガード）は全ドキュメントで旧説明をテキスト検索して更新する。
- **テンプレート内インラインハンドラの複雑さ** `[新観点 from PR#691]` — @イベントハンドラが2文以上のインライン関数になっている場合はscript内に名前付き関数として抽出する。他ページとの一貫性を保つ。
- **useTemplateRefパターンの使用** `[新観点 from PR#694]` — テンプレート参照には `ref()` ではなく `useTemplateRef()` を使用する。Vue 3.5 で導入された推奨APIであり、新規コンポーネントでは既存パターンとの整合性を確認する。
- **Teleportスタイルスコーピング** `[新観点 from PR#694]` — `<Teleport to="body">` を使用する場合、`<style scoped>` のデータ属性スコーピングが効かないためグローバルスタイルとなる。Teleport を使う際は必ずクラス名にコンポーネント固有のプレフィックスを付与するか、CSS Modules を使用する。
- **shared/utils vs shared/lib の配置基準** `[新観点 from PR#694]` — `shared/utils` は純粋関数のみ配置する（CODING_STANDARDS.md 準拠）。DOM操作やブラウザAPI副作用を持つ関数は `shared/lib` に配置する。新規ユーティリティ追加時に副作用の有無を確認する。
