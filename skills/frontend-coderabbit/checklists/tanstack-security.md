...
- **signalの受け取りと伝播** — `queryFn`/`ensureQueryData`は常に`signal`を受け取り下流のfetch関数まで伝播してキャンセルに対応する。`queryFn: () => fetch()`ではなく`queryFn: ({ signal }) => fetch({ signal })`とする
- **QueryKeyのシリアライズ可能性とundefined混入** — QueryKeyに`Date`,クラスインスタンス,`function`,`undefined`,`Symbol`を含めない。`undefined`はキャッシュ汚染の原因。オプショナルparamsにはデフォルト値を設定する
...
- **Mutation後のinvalidateと重複refetch回避** — Mutationの`onSuccess`で関連QueryKeyを`invalidateQueries`しているか。`invalidateQueries`だけでTanStack Queryが自動再フェッチするため、さらに手動で`await query.refetch()`を呼ぶと二重更新になる。重複した手動refetchが残っていないか確認する
...
- **keepPreviousDataの適用範囲とstaleデータリスク** — keepPreviousData（`placeholderData: keepPreviousData`）はリスト系クエリにのみ適用し、詳細系クエリ（ID指定の単一取得）には使わない（パラメータ切替時に別エンティティの古いデータが表示される）。QueryKeyにcompanyId等のテナント識別子を含むクエリでは切替時に前テナントのデータが一時表示されるリスクがあり、マスターデータ以外での使用は要注意。`:key="$route.path"`等で再マウントされるページではコンポーネント内のkeepPreviousDataは無効になるため、無意味な指定が残っていないか確認する
- **APIレスポンス必須フィールド検証** — 後続処理の前提となるレスポンスフィールド（ID等）の存在チェックが抜けていないか確認する。サーバーが予期しないレスポンスを返した場合に状態不整合やサイレント障害の原因になる。`== null`チェックでガードする
- **watch + TanStack Query の immediate: true** — watchでリアクティブな状態変化を監視する場合、初期値も処理対象かどうかを必ず検討し、必要なら`immediate: true`を付ける。特にキャッシュからの復元がありえるTanStack Queryとの組み合わせでは、マウント時に既にエラー状態の可能性がある
- **isLoading/isFetching/enabledの状態誤認** — 背景再取得中は`isLoading=false`だが`isFetching=true`になるため、初期選択やpending判定で`!isLoading && !error`を使う箇所は`!isLoading && !isFetching && !error`にする（staleキャッシュで初期選択が誤確定するリスク）。また`enabled: computed(...)`でdisabled状態のときは`isLoading=false, isFetching=false`になるため、watchでロード完了を判定する際は「クエリ未開始（disabled）」と「完了してデータが空」を区別しているか確認する
- **QueryKeyアサーションのハードコード禁止** — テストで`queryKey: ["journals"]`のようなリテラル配列を直書きしていないか確認する。QueryKey Factory定数（`journalKeys.all`等）を参照しないと、定数側の変更時にテストが偽陽性になる。`toHaveBeenCalledWith({ queryKey: xxxKeys.yyy })`の形式を使う
...
- **Open Redirect 対策** — 遷移先URL（リダイレクト先・`redirect` クエリ等）を外部入力から受け取る場合、許可リスト（同一オリジン/既知パス）で検証してから遷移する。検証なしの`router.push`/`location.href`代入は外部サイト誘導に悪用される（規約 MUST）
- **APIエンドポイント定数化** — `api.get('/api/...'`のようなURL文字列直書き禁止。定数/関数化必須
- **VITE_*に秘密情報禁止** — `.env*`ファイルに公開情報以外を置いていないか
- **秘密情報をAPI型/スキーマに保持しない** — APIレスポンス型・Zodスキーマ（`*Schema.ts`/`*ApiTypes.ts`）やentity型に、秘密値フィールド（各種トークン・パスワード・APIキー等）を含めていないか。バックエンドがレスポンスに含めていてもフロント型に残すと、表示・ログ・sessionStorage等への永続化で露出する。秘密情報非依存で動作させること
- **機密情報・PIIのログ出力禁止** — logger呼び出しや`console.*`の引数に個人識別情報（PII）やAPIキー・トークン・パスワード等の機密情報を含めていないか
...
- **write API の二重送信防止ガード** — mutation を呼ぶ関数には、UI の disabled 属性だけでなく関数側でも`isPending`/cooldown フラグによる早期 return を入れる。disabled はテンプレート側の制御であり、プログラム的な呼び出しを防げない
- **startsWith空文字ガード** — `startsWith("")`が常に`true`を返す仕様を意識し、プレフィックスベースの操作関数には空文字ガードを必ず入れる
