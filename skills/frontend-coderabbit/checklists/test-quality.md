...
- **未使用importの残存** `[新観点 from PR#736]` — テストファイルのimport文に未使用のモック関数が残っていないか。コピペでテストを作成した際に不要なimportが残りやすい。lintで自動検出されるが、テストファイルがlint対象から外れていないか確認する。
- **テストモックデータの重複定義** `[新観点 from PR#736]` — 同一テスト関数内で同じ値（ID, 名前等）が複数箇所に出現する場合、定数に抽出して一元管理しているか。重複定義は将来の変更時に片方だけ修正するリスクがある。
- **VRTのgoto直後のスクリーンショット** `[新観点 from PR#736]` — `page.goto` 直後に `toHaveScreenshot` を呼んでいないか。CI環境では描画完了前にスクリーンショットが撮られ不安定になる。代表要素の `toBeVisible` 待機を挟むこと。
- **テストURLとモックデータのID不整合** `[新観点 from PR#736]` — テストのURL内のIDとモックデータのデフォルトIDが一致しているか。ワイルドカードモックでテストが通っても意味的に不正確。
- **テストセレクタのCSSクラス直参照** `[新観点 from PR#736]` — テストコードで `.class-name` セレクタを使っていないか。CSSクラスはスタイリング目的であり、テストの安定性を担保するものではない。`data-testid` を使うこと。
- **VRT複数要素待機にtoHaveCount使用** `[新観点 from PR#736]` — VRTで複数要素の表示を待つ場合、`first().toBeVisible()` ではなく `toHaveCount()` で全件の描画完了を待機する。1件だけ待つとレースコンディションでスクリーンショットが不安定になる。
- **テストヘルパー引数差分の共通関数抽出** `[新観点 from PR#736]` — テストヘルパー関数で、引数1つだけ異なる同一フローが重複している場合は共通関数に抽出する。UI遷移フローの変更時に片方だけ更新漏れするリスクを防ぐ。
