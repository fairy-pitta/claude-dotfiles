...
- **モック定義とアサートの整合性** `[新観点 from PR#623]` — テストでモックを定義しているがアサートしていない箇所がないかチェックする。モックを作る＝検証する意図があるはず。アサートなしのモックはテストの意図が不完全。
- **エラーハンドリング分岐のテストカバレッジ** `[新観点 from PR#623]` — catch内の条件分岐（握りつぶす vs 再throw）がテストでカバーされているか確認する。特にcatch内の条件分岐は両方のパスをテストすべき。
...
- **テストスタブの自動emit抑制** `[新観点 from PR#623]` — テストスタブがコンポーネント内部のロジック（モーダル閉じ等）を自動的にbypassしていないか確認する。スタブはイベントのみemitし、副作用は実装側に委ねる。
- **2段階保存パスのテストカバレッジ** `[新観点 from PR#623]` — 条件分岐で保存APIが複数回呼ばれるフローがある場合、主要分岐のテストケースを追加する。
- **mutation onSuccess全分岐パスの検証** `[新観点 from PR#586]` — mutationのonSuccessで必ず実行される処理（invalidateQueries等）が、全分岐パス（regionId nullなど一部スキップする分岐含む）でテストされているか確認する。分岐で「実行される処理/されない処理」を明示的にリストアップして検証する。
- **MSWハンドラーの共通化** `[新観点 from PR#586]` — MSWハンドラーはドメイン別ファイル(`tests/mocks/handlers/`)に集約する。新規テスト作成時に同一エンドポイントのハンドラーがinlineで定義されていないか、既存ハンドラーファイルに追加すべきかを確認する。
- **MSWハンドラーAPIパス一貫性** `[新観点 from PR#642]` — 同一エンドポイントのMSWハンドラーが複数箇所で定義されている場合、APIパスプレフィックス（`/api/`等）の一貫性をチェックする。不一致があるとテストが実際のAPIと異なるパスでモックされ、偽陽性テストになる。grepで全パスを確認する。
