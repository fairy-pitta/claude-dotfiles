...
- **immediate watchの空初期値ガード** `[新観点 from PR#623]` — `immediate: true` の watch で初期値が空のケースを想定しているかチェックする。リアクティブなデータソースが非同期で到着する場合、immediate watch は空状態で先行実行され、後続の復元フローを破壊する可能性がある。
- **状態クリア関数の副作用順序** `[新観点 from PR#623]` — シングルトンストアの `reset()` が永続化をトリガーする副作用を持つか確認する。状態クリア系の関数を呼ぶ際は、副作用の実行順序に注意。reset → clear の順序で、clear が後続の persist タイマーをキャンセルする設計にする。
- **イベントファンアウトの例外隔離** `[新観点 from PR#623]` — イベントファンアウト（複数コールバック呼び出し）で1件の例外が他を止めないか確認する。`for...of` でコールバックを直接呼ぶパターンは、try/catch による隔離がないと全体が止まる。
- **catchブロックのエラー握りつぶし** `[新観点 from PR#623]` — 意図的にcatch {}でエラーを握りつぶす箇所で、本当にログ不要か検討する。複数ハンドラが登録される箇所ではどのハンドラで失敗したかの情報がないとデバッグ困難。console.*禁止のためshared/lib/logger（内部でSentryに送信）を使用する。
- **v-model双方向バインディングのイベント二重発火** `[新観点 from PR#623]` — 内部から値を変更すると`@update:model-value`ハンドラーも呼ばれることを意識する。モーダルの閉じ操作経路が複数ある場合、各経路からのイベント発火が重複しないか検証する。
- **エラーメッセージ定数の整合性** `[新観点 from PR#623]` — `ERROR_CODES`に新しい定数を追加する際、`USER_ERROR_MESSAGES`と`ERROR_CODE_TO_STATUS`にも必ず対応エントリを追加する。3つのマップの整合性を確認する。
- **catchブロックのエラーメッセージ種別** `[新観点 from PR#623]` — try-catchが分割されている場合、各catchブロックのエラーメッセージが対応するエラー種別を正しく反映しているか検証する。
