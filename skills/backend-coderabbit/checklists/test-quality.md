...
- **フィクスチャの重複定義** `[新観点 from PR#622]` — フィクスチャ追加時に親conftest.pyに同名フィクスチャが存在しないか確認する。pytestはディレクトリ階層でフィクスチャを解決するため、同名の再定義は暗黙のオーバーライドとなり修正漏れリスクを生む。親conftestを検索してから追加すること
- **異常系テストで副作用の非発生を検証** `[新観点 from PR#624]` — 早期リターンパスのテストでは、後続の依存メソッドが呼ばれないことを `assert_not_called()` で確認する。
- **テストヘルパーの暗黙None許容** `[新観点 from PR#624]` — テストセットアップ関数で不正な引数組み合わせ（例: REGION scopeなのにregion_id=None）を黙認しないようバリデーションを入れる。
- **テストの期待値と実装の整合性** `[新観点 from PR#624]` — エンドポイントの入力バリデーション仕様を変更した際、既存テストの期待値も合わせて更新する。
- **未使用パラメータの混入** `[新観点 from PR#624]` — 関数シグネチャ変更時に、渡された引数が実際に使われているか確認する。
- **ハードコードIDの回避** `[新観点 from PR#624]` — テストでフィクスチャ生成したオブジェクトのIDを使うべき。`region_id=1` 等のハードコードは避ける。
- **N+1クエリ検証の統一** `[新観点 from PR#624]` — 同ファイル内の類似テストが`django_assert_num_queries`を使用している場合、新規テストにも同等のクエリ数検証を追加すること。DB走査するエンドポイントのテストではN+1検出が標準装備であるべき。類似テストのパターンをgrepで確認する。
