...
- **ドキュメント内権限数の横断更新** `[新観点 from PR#670]` — 権限を追加・分離した際、同一ドキュメント内の全セクション（一覧・マッピング・サマリー）を横断的に更新しているか確認。エンドポイント表のみ修正し下部セクションの存在を失念しやすい。権限変更時にドキュメント全体で旧権限名・旧個数をテキスト検索する。
