...
- **論理削除エンドポイントのREST規約準拠** `[新観点 from PR#519]` — 論理削除を実装する際、DELETE メソッドを使用しREST規約に準拠しているか確認する。POST /deactivate/ のようなアクションベースURLではなく DELETE /resources/{id}/ が標準的。
- **ドキュメントと実装の権限コード同期** `[新観点 from PR#622]` — 権限コードを追加・削除・リネームした際に、計画ドキュメント・リファレンスのサマリーテーブルが更新されているか確認する
- **未使用関数パラメータ** `[新観点 from PR#622]` — 関数シグネチャに存在するがボディで未使用のパラメータは、API契約の誤解を招く。定義したパラメータが内部で使われているか確認する
- **ドキュメント凡例の明確性** `[新観点 from PR#622]` — ドキュメントの凡例・記号説明がコンテキストなしで読む人にも意味が伝わるかセルフレビューで確認する。略称・記号を使う場合は初出で定義を明示すること。曖昧な凡例は誤読を招く
- **テーブルセル内の主語省略** `[新観点 from PR#622]` — テーブルセル内の説明文で主語を省略していないか確認する。特に差分説明（「Xから〜を除外」等）は主語を明示しないと意味が曖昧になる。セル単体で意味が通る文にすること
- **Django management commandの失敗伝播** `[新観点 from PR#633]` — `handle()`メソッドがエラーケースで正常returnしていないか確認。Djangoではhandle()から例外が伝播しないとexit status 0になり、CI/CDがエラーを検出できない。失敗時は`CommandError`をraiseすること。
